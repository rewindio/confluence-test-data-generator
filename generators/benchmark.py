...
class BenchmarkTracker:
    ...
    def extrapolate_time(self, target_content: int, current_content: int) -> dict[str, any]:
        """Extrapolate time for a larger dataset based on current rates.

        Args:
            target_content: Target number of content items (pages + blogposts)
            current_content: Number of content items in current run

        Returns:
            Dict with extrapolation results
        """
        if current_content <= 0:
            return {"error": "No content created to extrapolate from"}

        scale_factor = target_content / current_content
        ...
        for phase_name, metrics in self.phases.items():
            if metrics.items_created > 0 and metrics.duration_seconds > 0:
                ...
                if phase_name in ("pages", "blogposts"):
    ...
    def format_extrapolation(self, target_content: int, current_content: int) -> str:
        ...
        """
        data = self.extrapolate_time(target_content, current_content)
        ...
        lines = [
            "",
            "=" * 60,
            f"TIME EXTRAPOLATION FOR {target_content:,} CONTENT ITEMS",
            "=" * 60,
            f"Based on current run: {current_content:,} content items",
    ...
    def get_summary_report(self) -> str:
        ...
        pages_phase = self.phases.get("pages")
        footer_comments_phase = self.phases.get("footer_comments")

        lines.append("")
        lines.append("Key rates for extrapolation:")
        if pages_phase and pages_phase.items_per_second > 0:
            lines.append(
                f"  Pages: {pages_phase.items_per_second:.2f}/sec ({pages_phase.seconds_per_item:.2f}s per page)"
            )
        if footer_comments_phase and footer_comments_phase.items_per_second > 0:
            lines.append(f"  Footer Comments: {footer_comments_phase.items_per_second:.2f}/sec")
